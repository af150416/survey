...
public class AttributesDao implements IAttributes {
	...
	public ExecutingStatus setAttribute(Attributes attributes) {
		...
		if (jdbcTemplate.queryForObject(SQL_FOR_CHECKING_ATTRIBUTES_EXISTING_BY_GROUP_AND_ATTRIBUTE, Integer.class,
				attributes.getGroupId(), attributes.getAttribute()) > 0) {
	...
	public ExecutingStatus updateAttributes(Attributes attributes) {
		...
		if (jdbcTemplate.update(SQL_FOR_UPDATING__ATTRIBUTES_BY_ID, attributes.getGroupId(), attributes.getAttribute(),
				attributes.getId()) == 1) {
			return ExecutingStatus.SUCCEEDED;
		}
	...
	public ExecutingStatus deleteAttributes(Integer attributesId) {
		...
		if ((jdbcTemplate.queryForObject(SQL_FOR_CHECKING_ATTRIBUTES_EXISTING_BY_ID, Integer.class,
				attributesId) == 0)) {
			return ExecutingStatus.NOT_EXIST;
		}
	...
	public List<Attributes> getAttributesByGroup(Integer groupId) {
		return (List<Attributes>) jdbcTemplate.query(SQL_FOR_GETTING_ATTRIBUTES_BY_GROUP,
