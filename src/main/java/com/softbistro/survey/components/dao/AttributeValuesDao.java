...
public class AttributeValuesDao implements IAttributeValues {
	...
	private final static String SQL_FOR_CHECKING_ATTRIBUTE_VALUES_EXISTING_BY_VALUES = "";
	private final static String SQL_FOR_CHECKING_ATTRIBUTE_VALUES_EXISTING_BY_ID = "";
	...
	public ExecutingStatus setAttributeValues(AttributeValues attributeValues) {
		if (jdbcTemplate.queryForObject(SQL_FOR_CHECKING_ATTRIBUTE_VALUES_EXISTING_BY_VALUES, Integer.class,
				attributeValues.getAttributeId(), attributeValues.getParticipantId(), attributeValues.getValue()) > 0) {
			return ExecutingStatus.ALREADY_EXIST;
		}
		if (jdbcTemplate.update(SQL_FOR_SETTING_ATTRIBUTE_VALUES, attributeValues.getAttributeId(),
				attributeValues.getParticipantId(), attributeValues.getValue()) == 1) {
	...
	public AttributeValues getAttributeValuesById(Integer attributeValuesId) {
		if(jdbcTemplate.queryForObject(SQL_FOR_CHECKING_ATTRIBUTE_VALUES_EXISTING_BY_ID, Integer.class, attributeValuesId) == 0){
	...
	public ExecutingStatus updateAttributeValuesById(AttributeValues attributeValues) {
		...
		if (jdbcTemplate.update(SQL_FOR_UPDATING_ATTRIBUTE_VALUES_BY_ID, attributeValues.getAttributeId(),
				attributeValues.getParticipantId(), attributeValues.getValue(), attributeValues.getId()) == 1) {
			return ExecutingStatus.SUCCEEDED;
		}
		return ExecutingStatus.FAILED;
	}
	...
	public ExecutingStatus deleteAttributeValuesById(Integer attributeValuesId) {
		...
		if (jdbcTemplate.update(SQL_FOR_DELETING_ATTRIBUTE_VALUES_BY_ID, attributeValuesId) == 1) {
			return ExecutingStatus.SUCCEEDED;
		}
		return ExecutingStatus.FAILED;
	}
